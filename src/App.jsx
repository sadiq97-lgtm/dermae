...
useEffect(() => {
  ...
  const routedProductId=productRouteMatch?decodeURIComponent(productRouteMatch[1]):null;
  const isProductRoute=Boolean(routedProductId);

  useEffect(()=>{
    if(!routedProductId) return;
    const matchedProduct=allProducts.find(product=>String(product.id)===routedProductId);
    if(matchedProduct) setSelectedProduct(matchedProduct);
  },[routedProductId,allProducts]);
...
<ScrollStory tr={tr} />
    ...
    {selectedProduct&&<motion.div className={`modal-overlay ${isProductRoute?"product-page-overlay":""}`} onClick={closeProductView} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><motion.div className={`product-modal mobile-product-modal ${isProductRoute?"routed-product-page":""}`} onClick={e=>e.stopPropagation()} initial={{opacity:0,y:35,scale:.96}} animate={{opacity:1,y:0,scale:1}}><button className="modal-close" onClick={closeProductView}><X size={12}/></button><div className="modal-image">{selectedProduct.image?<img src={selectedProduct.image} alt={selectedProduct.name}/>:<div className="product-image-placeholder">Dermaé</div>}</div><div className="modal-content"><span className="product-category">{selectedProduct.category}</span><h2>{isArabic?selectedProduct.name_ar||selectedProduct.name:selectedProduct.name_en||selectedProduct.name}</h2><div className="rating"><Star size={15} fill="currentColor"/><span>{selectedProduct.rating}</span><small>({selectedProduct.reviews})</small></div><div className="modal-price">{formatIQD(selectedProduct.price)}</div><p>{isArabic?selectedProduct.description_ar||selectedProduct.description:selectedProduct.description_en||selectedProduct.description}</p>{selectedProduct.ingredients?.length>0&&<div className="ingredients"><h4>{tr("Key ingredients","المكونات الأساسية")}</h4><div>{selectedProduct.ingredients.map(i=><span key={i}>{i}</span>)}</div></div>}{isProductRoute&&<div className="product-page-details">
  <div><span>01</span><h4>{tr("The ritual","الروتين")}</h4><p>{tr("Use this product as part of a consistent skincare routine and follow the directions on its label.","استخدم هذا المنتج ضمن روتين عناية منتظم واتبع التعليمات الموجودة على العبوة.")}</p></div>
  <div><span>02</span><h4>{tr("Delivery","التوصيل")}</h4><p>{tr("Available for delivery across Iraq. Delivery is free on orders over 100,000 IQD.","متاح للتوصيل داخل العراق، والتوصيل مجاني للطلبات التي تتجاوز 100,000 دينار.")}</p></div>
</div>}
<button className="modal-add" onClick={(event)=>{addToCart(selectedProduct,event.currentTarget.closest(".product-modal"));if(!isProductRoute)setSelectedProduct(null)}}>{tr("Add to Shopping Bag","أضف إلى سلة التسوق")}</button>
{isProductRoute&&<button className="product-page-back" type="button" onClick={()=>navigate("/")}>{tr("Back to the collection","العودة إلى المجموعة")} <ChevronRight size={16}/></button>}</div></motion.div></motion.div>}
