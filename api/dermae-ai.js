...
function cleanProducts(products) {
  ...
  return products.slice(0, 30).map((product) => ({
    ...
    ingredients: Array.isArray(product.ingredients) ? product.ingredients.slice(0, 12).map(String) : [],
  }));
}

export default async function handler(request, response) {
  if (request.method !== "POST") return response.status(405).json({ error: "Method not allowed" });
  if (!process.env.OPENAI_API_KEY) return response.status(503).json({ error: "Dermaé AI is not configured" });
  ...
  try {
    const apiResponse = await fetch(OPENAI_URL, {
      ...
      body: JSON.stringify({
        ...
        instructions,
        input: [
          { role: "user", content: [{ type: "input_text", text: `CATALOGUE JSON:\n${JSON.stringify(catalogue)}` }] },
          ...safeMessages.map((message) => ({ role: message.role, content: [{ type: "input_text", text: message.content }] })),
        ],
    ...
    if (!apiResponse.ok) {
      ...
      return response.status(502).json({ error: "Dermaé AI request failed" });
    }
    const output = String(data.output_text || "").trim();
    const match = output.match(/PRODUCT_IDS:\s*([^\n]+)/i);
    const productIds = match && !/none/i.test(match[1]) ? match[1].split(",").map((id) => id.trim()).filter((id) => catalogue.some((product) => product.id === id)).slice(0, 3) : [];
    const reply = output.replace(/\n?PRODUCT_IDS:\s*[^\n]+/i, "").trim();
    return response.status(200).json({ reply: reply || (language === "ar" ? "أخبرني أكثر عن نوع الروتين الذي تبحث عنه." : "Tell me more about the routine you are looking for."), productIds });
  } catch (error) {
    console.error("Dermaé AI server error:", error);
    return response.status(500).json({ error: "Dermaé AI is temporarily unavailable" });
  }
}
